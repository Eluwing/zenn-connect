...
Vueの場合、ライブラリーではなくJavaScriptフレームワークと言うもので
...
ただ、過去FacebookチームでFluxと言うパータンを公開した時に
...
当時のFaceBookでは読んでない一つのハイライトを表示すること同時に読んでないメッセージの数を表示するようにしたかったが、
...
もちろん問題解決には正解がないので、両方メリット・デメリットがあるだけで
どっちでも良いかと思います。
